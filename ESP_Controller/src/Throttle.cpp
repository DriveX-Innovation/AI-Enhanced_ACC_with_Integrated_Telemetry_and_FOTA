...
Throttle::Throttle(adc_channel_t channel)
    : throttle_adc_handle(nullptr), throttle_channel(channel) // GPIO34 corresponds to ADC_CHANNEL_6
{
}
...
int Throttle::ReadThrottle()
{
    ...
    if (raw < MINIMUM_ADC)
        raw = MINIMUM_ADC;
    else if (raw > MAXIMUM_ADC)
        raw = MAXIMUM_ADC;

    int mapped = MINIMUM_DIGITAL + (raw * (MAXIMUM_DIGITAL - MINIMUM_DIGITAL)) / MAXIMUM_ADC;

    /* Safety clamp */
    if (mapped < MINIMUM_DIGITAL)
        mapped = MINIMUM_DIGITAL;
    if (mapped > MAXIMUM_DIGITAL)
        mapped = MAXIMUM_DIGITAL;
...
float Throttle::ReadPercentage()
{
    ...
    return ((float)(adc - MINIMUM_DIGITAL) * 100.0f) / (MAXIMUM_DIGITAL - MINIMUM_DIGITAL);
}

int Throttle::GetRpmFromThrottle()
{
    int adc = ReadThrottle();

    float norm =
        (float)(adc - MINIMUM_DIGITAL) /
        (MAXIMUM_DIGITAL - MINIMUM_DIGITAL);

    float rpm =
        MIN_SPEED +
        norm * (MAX_SPEED - MIN_SPEED);
