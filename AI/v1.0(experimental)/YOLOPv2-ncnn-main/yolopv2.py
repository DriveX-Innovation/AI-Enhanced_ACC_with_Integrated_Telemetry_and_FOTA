...
# ── Combined inference: ONE backbone pass, multiple outputs ──────────────────
def detect_combined(bgr, target_size, conf_thresh, want_vehicles=True, want_drivable=True):
    """
    Critical optimization: we create ONE extractor and call ex.input() ONCE.
    ncnn builds a single computation graph; the shared backbone layers run
    exactly once regardless of how many of det0/det1/det2/677/769 we extract
    afterward. This is why combining detection+segmentation costs barely
    more than segmentation alone — we're not duplicating the expensive part.
    """
...
def draw_results(frame, objects, ll_mask, da_mask, dots, dot_color,
    ...
    if da_mask is not None and da_mask.shape[0] >= 2:
        drivable = (da_mask[0] < da_mask[1])
        overlay = image.copy()
    ...
    if ll_mask is not None:
        ...
        ys, xs = np.where(lane_binary[::dots, ::dots])
        ys = ys*dots; xs = xs*dots
        for x, y in zip(xs, ys):
            cv2.circle(image, (int(x),int(y)), 2, dot_color, -1, lineType=cv2.LINE_AA)

    for obj in objects:
        x,y,bw,bh = int(obj['x']),int(obj['y']),int(obj['w']),int(obj['h'])
        cv2.rectangle(image,(x,y),(x+bw,y+bh),(0,255,255),2)
        label=f"{obj['prob']*100:.0f}%"
        (lw,lh),bl=cv2.getTextSize(label,cv2.FONT_HERSHEY_SIMPLEX,0.45,1)
        ty=max(y-lh-bl,0); tx=min(x,w-lw)
        cv2.rectangle(image,(tx,ty),(tx+lw,ty+lh+bl),(255,255,255),-1)
        cv2.putText(image,label,(tx,ty+lh),cv2.FONT_HERSHEY_SIMPLEX,0.45,(0,0,0),1)
...
def run_image(image_path, model_dir, size, dots, color, conf, want_v, want_d):
    ...
    print(f"Saved result_ultimate.jpg  ({len(objects)} vehicles)")
    cv2.imshow("YOLOPv2 Ultimate", result)
    cv2.waitKey(0)
...
def run_camera(cam_id, model_dir, size, dots, color, conf, want_v, want_d):
    ...
    latest_frame  = [None]
    latest_result = [[], None, None]   # [objects, ll, da]
    lock = threading.Lock()
    ...
    while True:
        ...
        cv2.imshow("YOLOPv2 Ultimate  (q=quit  s=snapshot)", display)
