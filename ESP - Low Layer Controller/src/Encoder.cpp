...
EncoderPCNT_channel_PinA(nullptr), EncoderPCNT_channel_PinB(nullptr), lastCount(0), lastTime_us(0)
{
}
...
int Encoder::CalculateSpeed()
{
    ...
    if ((dt_us) < SAMPLE_PERIOD_US)
    {
        return -1; // No new sample yet
    }
    lastTime_us = now_us;

    int current = GetEncoderCount();
    int delta = abs(current - lastCount);
    lastCount = current;
    ...
    rpm_motor /= GEAR_RATIO;

    if (rpm_motor >= MAX_SPEED)
        rpm_motor = MAX_SPEED;
    else if (rpm_motor <= MIN_SPEED)
        rpm_motor = MIN_SPEED;
    
    return (int)(rpm_motor);
}
...
int Encoder::GetEncoderSpeedDigitalOutput()
{
    ...
    return (uint16_t)(speed * ((MAX_DIGITAL_OUTPUT - MIN_DIGITAL_OUTPUT) / (MAX_SPEED - MIN_SPEED)) + MIN_DIGITAL_OUTPUT);
}
