#!/usr/bin/env python3
"""
Merged ADAS: YOLOv8n + YOLOPv2 + Lane Curvature Analysis

SIMPLIFIED OUTPUT: Only shows cut_in and lane_curve variables
- cut_in: 1 if vehicle detected in ego lane, 0 otherwise
- lane_curve: 1 if lane is curved (R < 200m), 0 if straight
"""
...
MAX_STRIDE = 64
# Shared output — readable by external scripts
adas_output = {
    "cut_in":     0,
    "lane_curve": 0,
}
...
def main():
    ...
    print("[INIT] Starting webcam stream...")
    ...
    def infer_thread():
        print("[INIT] Loading YOLOv8n NCNN model...")
        yolo = YOLO(YOLOV8_MODEL_PATH, task='detect')
        
        print("[INIT] Loading YOLOPv2 NCNN model...")
        pv2_net = ncnn.Net()
        ...
        print("[INIT] Models loaded and warmed up successfully.\n")
    ...
    print("="*60)
    print("ADAS RUNNING - Simplified Output Mode")
    print("="*60)
    print("Output: [TIMESTAMP] cut_in=X | lane_curve=Y\n")
        ...
        while True:
            ...
            if result_payload is not None:
                ...
                if ll is not None:
                    ...
                    # Scan lower half of image to build Ego Lane polygon
                    ...
                    # Determine lane curve status
                    # lane_curve = 1 if lane is curved (radius < 200m)
                    # lane_curve = 0 if lane is straight (radius >= 200m)
                    if curv_info['valid']:
                        radius = curv_info['radius_m']
                        lane_curve = 1 if radius < 200 else 0

            
            # OUTPUT: Only show cut_in and lane_curve
            print(f"[{time.strftime('%H:%M:%S')}] cut_in={cut_in} | lane_curve={lane_curve}")
            adas_output["cut_in"]     = cut_in
            adas_output["lane_curve"] = lane_curve

            # Write to shared file
            with open('/tmp/adas_output.txt', 'w') as f:
