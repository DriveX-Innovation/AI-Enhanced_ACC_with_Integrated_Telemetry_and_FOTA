"""
send_ms.py — Stage 2  (final)
- COM8 : sends current speed in m/s every 10ms (as int16, unit = mm/s)
- COM5 : reads "target,actual\n" CSV (RPM from ESP), converts to m/s for plot
- Live rolling plot in m/s
"""
...
# ── Physical constant ──────────────────────────────────────────────────────
R_METERS    = 0.034                          # wheel radius in metres
# v (m/s) = 2π·r·RPM / 60   →   RPM = v·60 / (2π·r)
RPM_TO_MS   = (2 * math.pi * R_METERS) / 60
MS_TO_RPM   = 60 / (2 * math.pi * R_METERS)
...
# ── RPM step settings (internal — wire carries m/s) ───────────────────────
MIN_RPM      = 0
MAX_RPM      = 140
STEP_SIZE    = 20
...
MAX_MS       = MAX_RPM * RPM_TO_MS   # ≈ 0.497 m/s — used for plot y-axis

# ── Plot ───────────────────────────────────────────────────────────────────
PLOT_WINDOW  = 600
t_buf        = deque(maxlen=PLOT_WINDOW)
target_buf   = deque(maxlen=PLOT_WINDOW)   # stored in m/s
actual_buf   = deque(maxlen=PLOT_WINDOW)   # stored in m/s
...
#  Converts current_rpm → m/s → packs as int16 (unit: mm/s, i.e. ×1000).
#  Steps the RPM value every STEP_PERIOD seconds independently.
# ══════════════════════════════════════════════════════════════════════════
def rpm_sender(ser_data: serial.Serial):
    ...
    while True:
        ...
        # Convert RPM → m/s → int16 (mm/s, i.e. ×1000)
        v_ms     = rpm * RPM_TO_MS
        v_mm_s   = int(round(v_ms * 1000))
        frame    = struct.pack("<h", v_mm_s)
...
#  ESP still sends "target_rpm,actual_rpm\n"; we convert both to m/s here.
# ══════════════════════════════════════════════════════════════════════════
def csv_reader(ser_log: serial.Serial):
    ...
    while True:
        try:
            ...
            target_rpm = int(parts[0])
            actual_rpm = int(parts[1])

            # Convert RPM → m/s for the plot
            target_ms = target_rpm * RPM_TO_MS
            actual_ms = actual_rpm * RPM_TO_MS

            t_now = time.perf_counter() - t_start
            t_buf.append(t_now)
            target_buf.append(target_ms)
            actual_buf.append(actual_ms)
            frame_count += 1

            if frame_count % 200 == 0:
                print(f"[READER] samples={frame_count:6d} | "
                      f"target={target_ms:.4f} m/s | actual={actual_ms:.4f} m/s | "
...
def animate(_frame, ax, line_target, line_actual, txt_status):
    ...
    ax.set_ylim(-0.02, MAX_MS + 0.05)
    with lock:
        rpm = current_rpm
    txt_status.set_text(f"Setpoint: {rpm * RPM_TO_MS:.4f} m/s\nSamples : {frame_count}")
...
def main():
    try:
        ...
        print(f"[OK] {DATA_PORT} — speed command port (UART2)")
    ...
    line_target, = ax.plot([], [], "b-",  linewidth=2.0, label="Target (m/s)")
    line_actual,  = ax.plot([], [], "r-", linewidth=1.5, label="Actual (m/s)")
